...
#define TRANSFER_ROUND_NO          2            /* The number of rounds sender will transmit */
...
void* receiver_task(void *param)
{
    ...
    struct libusb_device_handle **dev_handle = (struct libusb_device_handle **)param;   /* Structure of
		 the opened PL25A1 device handle */
...
void* sender_task(void *param)
{
    ...
    for (i = 0; i < TRANSFER_ROUND_NO - 1 && !gkilled; i++) {
        transferred_size = 0;
	...
	// Divide last 512 bytes into 511 bytes and 1 byte to ensure USB host sends short packet in the end.
	return_code = libusb_bulk_transfer(
		*dev_handle,
		BULK_USB2_EP1_OUT_ADDR,
		sendBuf,
...
{
    bool found_PL25A1_device = false;
    ...
    while((device = devices[i++]) != NULL) {
        ...
        // Find PL25A1 USB device
        if((PROLIFIC_VID == device_descriptor.idVendor) && (PL25A1_PID == device_descriptor.idProduct)) {
            found_PL25A1_device = true;
            DEBUG("(%s, %s(), L%d)\n", __FILE__, __FUNCTION__, __LINE__);
            printf("Found PL25A1 USB device!\n");
    ...
    // Check whether any PL25A1 USB device was found
    if(!found_PL25A1_device)
    {
        printf("No PL25A1 USB device was found!\n");
	...
	while (1)
	{
		...
		{
			printf("Fail to get PL25A1 USB device status!\n");
			return PL_ERROR_WRONG_STATUS;
		}
		printf("Local device status: %s, %s\n", dev_status.localSuspend ? "Suspend" : "Active", 
			dev_status.localUnplug ? "Unplug" : "Attached");
		printf("Remote device status: %s, %s\n", dev_status.RemoteSuspend ? "Suspend" : "Active",
			dev_status.RemoteUnplug ? "Unplug" : "Attached");
		// Break the loop when the remote device was attached
		if (!dev_status.RemoteUnplug)
			break;
#if !defined(_WIN32)
		usleep(SLEEP_TIME * 1000);
#else
