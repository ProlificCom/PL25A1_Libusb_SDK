...
//
//  Created by software team in Prolific on 11/11/2016.
//  Copyright (c) 2016 Prolific Corp.. All rights reserved.
//
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    ...
    while((device = devices[i++]) != NULL) {
        ...
        if((PROLIFIC_VID == device_descriptor.idVendor) && (PL25A1_PID == device_descriptor.idProduct)) {
            ...
            printf("--------------------------------------------------------------\n");
            printf("    PL25A1 USB device information\n");
                ...
                for(k = 0; k < interface_descriptor.bNumEndpoints; k++) {
                    ...
                    if(endpoint.bmAttributes == LIBUSB_TRANSFER_TYPE_BULK)
                    {
                        ...
                        if((endpoint.bEndpointAddress & LIBUSB_ENDPOINT_DIR_MASK)
                            ...
                            if(endpoint.bEndpointAddress != BULK_USB2_EP0_IN_ADDR &&
                                    endpoint.bEndpointAddress != BULK_USB2_EP1_IN_ADDR) {
                                DEBUG("(%s, %s(), L%d)\n", __FILE__, __FUNCTION__, __LINE__);
                                break;
                            }
                        }
                        ...
                        if((endpoint.bEndpointAddress & LIBUSB_ENDPOINT_DIR_MASK)
                            ...
                            if(endpoint.bEndpointAddress != BULK_USB2_EP1_OUT_ADDR) {
                                DEBUG("(%s, %s(), L%d)\n", __FILE__, __FUNCTION__, __LINE__);
                                break;
                            }
